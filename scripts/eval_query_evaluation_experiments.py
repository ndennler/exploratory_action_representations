...
from clea.reward_models.eval_utils import get_pids_for_training, get_train_test_dataloaders, train_single_epoch, eval_model, calc_reward, generate_embeddings
...
embedding_size = 64 # has to be the same as the training embedding size
seed=13
#########################################
#                                       #
...
for SIGNAL_MODALITY in ['visual', 'auditory', 'kinesthetic']:
    for EMBEDDING_TYPE in ['contrastive+autoencoder', 'contrastive','autoencoder','VAE', 'random']:
        for signal in ['idle', 'searching', 'has_item', 'has_information']:
            ...
            embedding_model = torch.load('../data/final_models/' + model_name, map_location=torch.device('cpu'))
            ...
            for pid in get_pids_for_training(SIGNAL_MODALITY, signal):
                ...
                if CALC_REWARD:
                    ...
                    training_sets, testing_sets = get_train_test_dataloaders(pid, SIGNAL_MODALITY, signal, raw_data=FROM_RAW_DATA, model_name=model_name[:-4])
            ...
            if os.path.exists('../data/results/TPA_results.csv'):
                ...
                pd.concat([df, pd.DataFrame(data)]).to_csv('../data/results/TPA_results.csv', index=False)
            else:
                pd.DataFrame(data).to_csv('../data/results/TPA_results.csv', index=False)
