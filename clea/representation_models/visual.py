...
class Pretrained2RawImageAE(nn.Module):
  ...
  def __init__(
    ...
    super(Pretrained2RawImageAE, self).__init__()
    self.device = device

    self.encoder = RawImageEncoder(output_dim, hidden_dim, latent_dim, device)
    self.decoder = RawImageDecoder(self.encoder.get_intermediate_size(), self.encoder.get_reshape_size(), hidden_dim, latent_dim, device)
...
class Pretrained2RawImageVAE(nn.Module):
  ...
  def __init__(
    ...
    super(Pretrained2RawImageVAE, self).__init__()
    self.device = device
    self.nz = latent_dim

    self.encoder = RawImageEncoder(output_dim, hidden_dim, 2*latent_dim, device)
    self.decoder = RawImageDecoder(self.encoder.get_intermediate_size(), self.encoder.get_reshape_size(), hidden_dim, latent_dim, device)
