...
def get_model_and_loss_fn(model_type: str, 
    ...
    if model_type == 'contrastive':
            return RawAudioEncoder(input_dim, hidden_dim, latent_dim, device=device), nn.TripletMarginLoss(margin=parameter)
            ...
            model = RawAudioVAE(input_dim, hidden_dim, latent_dim, device=device, beta=parameter)
...
def train(model_type, DEVICE='cpu ', BATCH_SIZE=128, EMBEDDING_DIM=128, LR=1e-4, NUM_EPOCHS=2, parameter=1):
        ...
        for signal in ['idle', 'searching', 'has_item', 'has_information']:
                print(f'Training auditory modality;  {signal} signal; {model_type} model;')

                #1. get dataloader
                data, input_dim = get_dataloader(batch_size=BATCH_SIZE, modality='auditory', signal=signal)    
                ...
                torch.save(model, f'../data/trained_models2/auditory&{model_type}&{signal}&{EMBEDDING_DIM}.pth')
...
def eval(model_type, EMBEDDING_DIM=128, DEVICE='cpu'):
        ...
        for sig in ['idle', 'searching', 'has_information', 'has_item']:
                embedding_model = torch.load(f'../data/trained_models2/auditory&{model_type}&{sig}&{EMBEDDING_DIM}.pth', map_location=torch.device(DEVICE))
                ...
                for PID in query_df['pid'].unique():
                        ...
                        train_data = RawQueryDataset(train_df, kind='auditory', transform=torch.Tensor, data_dir='../data/')
                        ...
                        test_data = RawQueryDataset(test_df, kind='auditory', transform=torch.Tensor, data_dir='../data/')
